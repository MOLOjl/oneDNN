...
status_t cudnn_multi_head_attn_bwd_data_t::execute(
        const exec_ctx_t &ctx) const {
    nvidia::stream_t *cuda_stream
            = utils::downcast<nvidia::stream_t *>(ctx.stream());
...
status_t cudnn_multi_head_attn_bwd_weights_t::execute(
        const exec_ctx_t &ctx) const {
    nvidia::stream_t *cuda_stream
            = utils::downcast<nvidia::stream_t *>(ctx.stream());
    ...
    return cuda_stream->interop_task([&](::sycl::handler &cgh) {
        ...
        auto arg_dweightspace = CTX_SCRATCH_SYCL_MEMORY(memory_tracking::names::key_attn_dweight);
        ...
        compat::host_task(cgh, [=, this](const compat::interop_handle &ih) {
            ...
            args.push_back(arg_values.get_native_pointer(ih));
            args.push_back(arg_dout.get_native_pointer(ih));
            args.push_back(arg_dweightspace.get_native_pointer(ih));
