...
status_t miopen_multi_head_attn_bwd_data_t::execute(
    ...
    return hip_stream->interop_task([&](::sycl::handler &cgh) {
        ...
        auto arg_dqueries = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 0);
        auto arg_dkeys = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 1);
        auto arg_dvalues = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 2);
...
status_t miopen_multi_head_attn_bwd_weights_t::execute(
    ...
    return hip_stream->interop_task([&](::sycl::handler &cgh) {
        ...
        auto arg_dqweight = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 0);
        auto arg_dqbias = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 1);
        auto arg_dkweight = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 2);
        auto arg_dkbias = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 3);
        auto arg_dvweight = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 4);
        auto arg_dvbias = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 5);
        auto arg_doweight = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 6);
        auto arg_dobias = CTX_OUT_SYCL_MEMORY(DNNL_ARG_MULTIPLE_DST + 7);
