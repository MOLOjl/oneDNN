...
status_t hip_gather_t::execute(const exec_ctx_t &ctx) const {
    ...
    return hip_stream->interop_task([&](::sycl::handler &cgh) {
        ...
        auto arg_idx = CTX_IN_SYCL_MEMORY(DNNL_ARG_WEIGHTS);
        ...
        compat::host_task(cgh, [=](const compat::interop_handle &ih) {
            ...
            void *index = arg_idx.get_native_pointer(ih);

            pd()->gather_impl_->execute(x, y, index);
        });
    });
}
