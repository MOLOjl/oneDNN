...
status_t hip_embedding_t::execute(const exec_ctx_t &ctx) const {
    ...
    return hip_stream->interop_task([&](::sycl::handler &cgh) {
        ...
        auto arg_dict = CTX_IN_SYCL_MEMORY(DNNL_ARG_WEIGHTS);
        ...
        compat::host_task(cgh, [=](const compat::interop_handle &ih) {
            ...
            void *dict = arg_dict.get_native_pointer(ih);

            pd()->embedding_impl_->execute(x, y, dict);
        });
    });
}
