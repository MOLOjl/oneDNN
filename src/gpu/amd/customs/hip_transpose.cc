...
__global__ void transpose_kernel(T1 *input, T1 *output, size_t* in_strides, size_t* out_strides, int num_dims, int dim1, int dim2, size_t total_elements)
{
...
void transpose(size_t byte_len, void *input, void *output, const size_t *dims, int num_dims, int dim1, int dim2)
{
  ...
  if(byte_len == 8)
    hipLaunchKernelGGL(transpose_kernel, dim3(numBlocks), dim3(blockSize), 0, 0, (int64_t*)input, (int64_t*)output, d_in_strides, d_out_strides, num_dims, dim1, dim2, total_elements);  
  if(byte_len == 4)
    hipLaunchKernelGGL(transpose_kernel, dim3(numBlocks), dim3(blockSize), 0, 0, (int32_t*)input, (int32_t*)output, d_in_strides, d_out_strides, num_dims, dim1, dim2, total_elements);
  if(byte_len == 2)
    hipLaunchKernelGGL(transpose_kernel, dim3(numBlocks), dim3(blockSize), 0, 0, (int16_t*)input, (int16_t*)output, d_in_strides, d_out_strides, num_dims, dim1, dim2, total_elements);
  if(byte_len == 1)
    hipLaunchKernelGGL(transpose_kernel, dim3(numBlocks), dim3(blockSize), 0, 0, (int8_t*)input, (int8_t*)output, d_in_strides, d_out_strides, num_dims, dim1, dim2, total_elements);
